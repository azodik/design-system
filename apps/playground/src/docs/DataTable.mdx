import { DataTable, Card, CardContent, Button } from '@azodik/ui';
...
DataTable is an enhanced table component that provides advanced functionality like sorting, filtering, selection, and pagination. It's ideal for displaying large datasets with interactive features.
...
### When not to use
- For simple static data (use Table instead)
...
<ComponentRenderer componentName="datatable" />
...
<CodeBlockWithPreview 
  title="DataTable with Custom Actions"
  preview={<ActionDataTableExample />}
>
{`import { DataTable, Button } from '@azodik/ui';
