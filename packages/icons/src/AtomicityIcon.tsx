import React from "react";

interface AtomicityIconProps {
  size?: number;
  className?: string;
  style?: React.CSSProperties;
}

const AtomicityIcon: React.FC<AtomicityIconProps> = ({ size = 20, className, style }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="none"
      width={size}
      height={size}
      className={className}
      style={{
        color: "currentColor",
        ...style,
      }}
    >
      <path
        fill="currentColor"
        d="M14.415 3.308c2.074-.736 4.31-.846 5.717.56c1.406 1.407 1.296 3.643.56 5.717A14.5 14.5 0 0 1 19.552 12c.478.82.862 1.632 1.14 2.415c.736 2.074.846 4.31-.56 5.717c-1.407 1.406-3.643 1.296-5.717.56A14.5 14.5 0 0 1 12 19.552a14.5 14.5 0 0 1-2.415 1.14c-2.074.736-4.31.846-5.717-.56c-1.406-1.407-1.296-3.642-.56-5.716c.278-.783.661-1.596 1.138-2.416c-.477-.82-.86-1.633-1.138-2.416c-.736-2.074-.846-4.31.56-5.716s3.642-1.296 5.716-.56c.783.278 1.596.661 2.416 1.138a14.5 14.5 0 0 1 2.415-1.138M5.703 13.884q-.304.619-.51 1.201c-.672 1.892-.457 3.087.09 3.633c.545.546 1.74.762 3.632.09q.582-.207 1.2-.512a22.5 22.5 0 0 1-2.358-2.054a22.5 22.5 0 0 1-2.054-2.358m12.593 0a22.5 22.5 0 0 1-2.054 2.36a22.5 22.5 0 0 1-2.358 2.052q.619.304 1.2.512c1.893.67 3.088.456 3.634-.09s.76-1.741.09-3.633q-.208-.583-.512-1.201M12 10a2 2 0 1 0 0 4a2 2 0 0 0 0-4M8.915 5.192c-1.892-.671-3.087-.456-3.633.09s-.761 1.74-.09 3.633q.207.582.511 1.2a22.5 22.5 0 0 1 2.055-2.357a22.5 22.5 0 0 1 2.357-2.055a11 11 0 0 0-1.2-.51m9.803.09c-.546-.546-1.741-.76-3.633-.09q-.583.207-1.201.511a22.5 22.5 0 0 1 2.358 2.054c.762.762 1.45 1.556 2.054 2.358q.304-.617.512-1.199c.67-1.892.456-3.088-.09-3.634"
      />
    </svg>
  );
};

export default AtomicityIcon;
