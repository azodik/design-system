import React from "react";

interface IstioIconProps {
  size?: number;
  className?: string;
  style?: React.CSSProperties;
}

const IstioIcon: React.FC<IstioIconProps> = ({ size = 20, className, style }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 128 128"
      fill="none"
      width={size}
      height={size}
      className={className}
      style={{
        color: "currentColor",
        ...style,
      }}
    >
      <path
        fill="currentColor"
        d="M58.802.013a.13.13 0 0 0-.073.11V101.73c0 .064.05.119.113.126l47.918 5.349a.13.13 0 0 0 .144-.114a.14.14 0 0 0-.014-.067L58.972.073a.127.127 0 0 0-.17-.06m-5.535 35.55a.127.127 0 0 0-.17.06l-31.99 71.4a.13.13 0 0 0-.01.07c.01.07.077.116.147.106l31.99-5.482a.12.12 0 0 0 .103-.123l.004-65.918a.13.13 0 0 0-.074-.114zM21.321 111.86a.12.12 0 0 0-.12.073a.126.126 0 0 0 .063.167l32.03 15.892c.03.01.064.01.093 0l53.006-15.892a.13.13 0 0 0 .07-.093a.126.126 0 0 0-.103-.147z"
      />
    </svg>
  );
};

export default IstioIcon;
