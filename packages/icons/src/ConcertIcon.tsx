import React from "react";

interface ConcertIconProps {
  size?: number;
  className?: string;
  style?: React.CSSProperties;
}

const ConcertIcon: React.FC<ConcertIconProps> = ({ size = 20, className, style }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="none"
      width={size}
      height={size}
      className={className}
      style={{
        color: "currentColor",
        ...style,
      }}
    >
      <g fill="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5">
        <path d="M9 6.75a3 3 0 1 0 6 0a3 3 0 0 0-6 0m-1.5 9a4.5 4.5 0 1 1 9 0z" />
        <path d="M6 9V6.75a6 6 0 1 1 12 0v1.5m4.437 13.069A1.5 1.5 0 0 1 21 23.25H3a1.5 1.5 0 0 1-1.437-1.931l1.35-4.5A1.5 1.5 0 0 1 4.35 15.75h15.3a1.5 1.5 0 0 1 1.437 1.069z" />
        <path d="M5.25 19.5c0 .398.237.78.659 1.06S6.903 21 7.5 21s1.169-.158 1.591-.44c.422-.28.659-.662.659-1.06s-.237-.78-.659-1.06S8.097 18 7.5 18s-1.169.158-1.591.44c-.422.28-.659.662-.659 1.06m9 0c0 .197.058.392.171.574s.28.347.488.487c.209.139.457.25.73.325s.566.114.861.114s.588-.039.861-.114s.521-.186.73-.325c.209-.14.375-.305.488-.487s.171-.377.171-.574s-.058-.392-.171-.574a1.6 1.6 0 0 0-.488-.487a2.5 2.5 0 0 0-.73-.325c-.273-.075-.565-.114-.861-.114s-.588.039-.861.114a2.5 2.5 0 0 0-.73.325a1.6 1.6 0 0 0-.488.487a1.1 1.1 0 0 0-.171.574M21 7.5a2.25 2.25 0 0 1-2.25 2.25A.75.75 0 0 1 18 9V5.25h.75A2.25 2.25 0 0 1 21 7.5m-18 0a2.25 2.25 0 0 0 2.25 2.25A.75.75 0 0 0 6 9V5.25h-.75A2.25 2.25 0 0 0 3 7.5" />
      </g>
    </svg>
  );
};

export default ConcertIcon;
