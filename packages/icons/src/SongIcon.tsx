import React from "react";

interface SongIconProps {
  size?: number;
  className?: string;
  style?: React.CSSProperties;
}

const SongIcon: React.FC<SongIconProps> = ({ size = 20, className, style }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 40 40"
      fill="none"
      width={size}
      height={size}
      className={className}
      style={{
        color: "currentColor",
        ...style,
      }}
    >
      <g fill="currentColor">
        <path
          fill="currentColor"
          strokeMiterlimit="10"
          d="M31.86 2a7.34 7.34 0 0 0-4.52 1.85l2.41 3.76L28 10.69l2.5 2.77l-2.87 4l-.23-3.27L24 11.8l1.52-3.88l-1.39-3.47a7.2 7.2 0 0 0-4.88-.15c-4.5 1.24-6 4.56-4.89 8.61C16.08 19.14 23 21.86 28.1 23a3.15 3.15 0 0 0 2-.37c4.4-2.85 9.9-7.83 9.36-14.26c-.37-4.19-2.95-6.77-7.6-6.37Z"
        />
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M33.34 3.67a3.55 3.55 0 0 1 3.14 1.5"
        />
        <path
          fill="currentColor"
          strokeMiterlimit="10"
          d="M27.65 22.1c0-.94-.5-1.59-1.89-2a38.4 38.4 0 0 0-7-.64c-2.91-.08-6.7 0-7.55.54a4.16 4.16 0 0 0-1.45 2.3c-.43 1.42-1 3.36-1.45 5c-3.08-.59-5.11.95-5.84 3.81s.38 5.44 3.73 6.3s5.52-.72 6.3-3.72l1.28-5c.1-.36.19-.74.29-1.13c1.19.12 2.57.2 3.82.24a5 5 0 0 0-2.87 3.64c-.77 3 .39 5.44 3.73 6.3s5.4-.75 6.3-3.73c.75-2.24 2.51-7.78 2.6-11.91Z"
        />
        <path strokeLinecap="round" strokeMiterlimit="10" d="M17.94 27.86a6 6 0 0 1 3.33 0" />
        <path strokeLinecap="round" strokeMiterlimit="10" d="M22.49 21.49c2.92.17 3.21.3 3.23.77" />
        <path
          fill="currentColor"
          strokeMiterlimit="10"
          d="M7.69 5.2c-.95.3-2.35.8-.69 4.6s2.65 3.57 3.12 3.43s1.42-.44.65-4.6S8.63 4.91 7.69 5.2Zm1.89 10.75a1.48 1.48 0 1 0 2.96 0a1.48 1.48 0 0 0-2.96 0Zm-8.27-5.66c-.66.42-1.63 1.07.46 3.64s2.77 2.18 3.1 2s1-.63-.46-3.64s-2.41-2.42-3.1-2ZM5 18.14a1.17 1.17 0 1 0 2.34 0a1.17 1.17 0 0 0-2.34 0Z"
        />
      </g>
    </svg>
  );
};

export default SongIcon;
