...
<CodeBlockWithPreview
  ...
  preview={
    <div style={{ position: "relative", height: "16rem", width: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "var(--space-4)", border: "1px solid var(--gray-6)", borderRadius: "var(--radius-2)", backgroundColor: "var(--gray-2)" }}>
       <div style={{ textAlign: "center", display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
          <p style={{ fontSize: "var(--font-size-sm)", color: "var(--color-text-secondary)" }}>The panel is usually fixed to the screen edge, but demonstrated here statically.</p>
