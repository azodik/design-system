import { DataTable, Card, CardContent, Button, Pagination, Flex, Stack, Center } from '@azodik/ui';
...
export const ActionDataTableExample = () => {
  ...
  const columns = [
    ...
    { 
      ...
      render: (row) => (
        <Flex gap="2">
          <Button size="xs" variant="outline" onClick={() => console.log('Edit:', row.id)}>
            Edit
          </Button>
          <Button size="xs" variant="solid" color="ruby" onClick={() => console.log('Delete:', row.id)}>
            Delete
          </Button>
        </Flex>
      )
...
export const PaginatedDataTableExample = () => {
  ...
  return (
    <div style={{ width: '100%', overflow: 'hidden' }}>
      ...
      <Center style={{ marginTop: "var(--space-4)" }}>
...
<CodeBlockWithPreview 
  ...
  preview={
    <Stack gap={16} style={{ width: '100%', overflow: 'hidden' }}>
      <div style={{ width: '100%', overflow: 'hidden' }}>
        <DataTable 
          data={[
            { id: 1, name: 'John Doe', email: 'john@example.com', role: 'Admin' },
